...
public class FlipFitGymCustomerDaoImpl {
    ...
    public void setPreferredCity(int userId, String city) throws CustomerNotRegisteredException {
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            if (rowsUpdated == 0) {
                throw new CustomerNotRegisteredException("Customer with ID " + userId + " not found.");
            }
            System.out.println("Preferred city for user " + userId + " set to " + city);
        } catch (SQLException e) {
            throw new CustomerNotRegisteredException("Error updating preferred city: " + e.getMessage(), e);
        }
    }

    // 3. Book a Slot
    public void bookSlot(FlipFitSlotBooking booking) throws SlotUnavailableException {
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            throw new SlotUnavailableException("Slot booking failed: " + e.getMessage());
        }
    }
    ...
    public void listAllCentersByCity(String city) {
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                ...
                System.out.println("Center ID: " + centerId + ", Name: " + name + ", Address: " + address + ", City: "
                        + centerCity + ", Seats/Hour: " + seatsPerHour);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void viewAvailableSlots(int gymCenterId) throws GymCenterNotFoundException {
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                ...
                System.out.println(
                        "Slot ID: " + slotId + ", Slot Info: " + slotInfo + ", Available Seats: " + availableSeats);
            }
        } catch (SQLException e) {
            throw new GymCenterNotFoundException("Error in finfing the gym");
        }
    }
    ...
    public void viewBookedSlots(int userId) throws CustomerNotRegisteredException {
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            throw new CustomerNotRegisteredException("Error in finding Customer");// customer not
        }
    }

    // 8. Get Payment Status (query payment table)
    ...
    // 9. Send Notification (insert into notification table)
